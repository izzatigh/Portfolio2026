...
function getNavClasses({ isActive }) {
  return [
    'font-bold uppercase tracking-widest text-xs transition-colors hover:text-[#4d6953]',
    isActive ? 'text-[#4d6953]' : 'text-ink',
  ].join(' ');
}
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border bg-canvas/95 backdrop-blur-sm">
      <Container className="flex items-center justify-between gap-6 py-4">
        <NavLink
          className="font-display text-lg font-bold uppercase tracking-widest text-ink hover:text-[#4d6953] transition-colors"
          style={{ letterSpacing: '0.18em' }}
          to="/"
        >
          Izzati<span style={{ color: '#c94b2b' }}>.</span>
        </NavLink>
        <nav className="flex flex-wrap items-center gap-6 text-xs">
          ...
          <a
            aria-label="LinkedIn"
            className="text-muted transition-colors hover:text-[#4d6953]"
